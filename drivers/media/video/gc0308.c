...
#if defined(CONFIG_FCAM_NEXT8PSEHD) || defined(CONFIG_MACH_RK29_AOSONM12_512M)
			{0x14, 0x10},
#else
			{0x14, 0x13},
#endif
...
static ssize_t gc0308_show(struct class *cls, char *_buf)
{
	return printk("%s()\n", __FUNCTION__);
}
...
static ssize_t gc0308_store(struct class *cls, const char *_buf, size_t _count)
{
	const char * p=_buf;
	u8 reg;
	u8 val;
...
static struct class *gc0308_class = NULL;
static CLASS_ATTR(gc0308, 0666, gc0308_show, gc0308_store);
			 ...
			 const struct i2c_device_id *did)
{
    ...
    gc0308_class = class_create(THIS_MODULE, "gc0308");
    if (IS_ERR(gc0308_class)) 
    {
        printk("Create class gc0308.\n");
        return -ENOMEM;
    }
    class_create_file(gc0308_class,&class_attr_gc0308);
    g_i2c_client=client;
...
struct cam_sensor_info cam_sensor_info_gc0308={
    "gc0308",
    1,
    1,
    0x42>>1,
...
EXPORT_SYMBOL_GPL(cam_sensor_info_gc0308);
