...
static int goodix_init_panel(struct goodix_ts_data *ts)
{
...
#ifdef DRIVER_SEND_CFG				//for kedi 9.7
	uint8_t config_info[] = {
	...
	(TOUCH_MAX_WIDTH>>8),(TOUCH_MAX_WIDTH&0xff),MAX_FINGER_NUM,(0x3C |INT_TRIGGER),
	0x1E,0xE7,0x32,0x02,0x08,0x10,0x4C,0x47,
	0x47,0x20,0x03,0x00,0x60,0x60,0x3C,0x96,0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,	
	0x08,0x09,0x0A,0x0B,0x0C,0x0E,0x0D,0x0F,0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,
	0x18,0x19,0x1A,0x1B,0x1D,0x1C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00
	};
	#endif
	#if 0			//for malata 10.1
	uint8_t config_info[] = {
	0x65,0x03,(TOUCH_MAX_HEIGHT>>8),(TOUCH_MAX_HEIGHT&0xff),
	(TOUCH_MAX_WIDTH>>8),(TOUCH_MAX_WIDTH&0xff),MAX_FINGER_NUM,(0x3C |INT_TRIGGER),
	0x14,0x14,0x00,0x02,0x08,0x19,0x00,0x20,
	0x00,0x10,0x10,0x10,0x00,0x37,0x3C,0x64,0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
	0x08,0x09,0x0A,0x0B,0x0C,0xFF,0x0D,0x0E,0x0F,0x10,0x11,0x12,0x13,0x14,0x15,0x16,
	0x17,0x18,0x19,0x1A,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00
	};
	#endif
...
#if 0
	uint8_t config_info[] = {
	0x68,(TOUCH_MAX_WIDTH>>8),(TOUCH_MAX_WIDTH&0xff)
	};
...
return:
	Executive outcomes.0---succeed.
*******************************************************/
char figer_old[MAX_FINGER_NUM]={0};
char figer_new[MAX_FINGER_NUM]={0};
char get_keystatus[MAX_FINGER_NUM]={0};
int input_buf[MAX_FINGER_NUM][3]= {0};
int key_num = -1;
int point_num=0;
...
{	
	...
	//int input_y[MAX_FINGER_NUM] = {0};
	//int input_w[MAX_FINGER_NUM] = {0};
...
//	printk("finish the i2c communication!\n");
	...
	{	
		...
		check_sum = point_data[2 - READ_COOR_ADDR] + point_data[3 - READ_COOR_ADDR]; 	//point_data[2]+point_data[3]		//cal coor checksum
		count = 4 - READ_COOR_ADDR;
		for(point_count *= 5; point_count > 0; point_count--)
			check_sum += point_data[count++]; 											//point_data[4]+
	...
	if(touch_num)
	{
		for(index=0; index<touch_num; index++)
		{
			...
			input_x = ts->abs_x_max - ((unsigned int)(point_data[position]<<8) + (unsigned int)( point_data[position+1]));
			input_y = ts->abs_y_max - ((unsigned int)(point_data[position+2]<<8) + (unsigned int)(point_data[position+3]));
			...
			input_mt_slot(ts->input_dev, touch_num);
	...
	figer_new[4]=((point_data[1]&0x1f)>>4)&0x01;
	figer_new[3]=((point_data[1]&0x1f)>>3)&0x01;
	figer_new[2]=((point_data[1]&0x1f)>>2)&0x01;
	figer_new[1]=((point_data[1]&0x1f)>>1)&0x01;
	figer_new[0]=(point_data[1]&0x1f)&0x01;
	//printk("figer_new[4] = %d,figer_new[3] = %d, figer_new[2] = %d,figer_new[1]=%d,figer_new[0]=%d\n", figer_new[4], figer_new[3], figer_new[2],figer_new[1],figer_new[0]);
	for(index=0;index<5;index++)
		{
			if((figer_old[index]==0)&&(figer_new[index]==1))
				{
					//表示按下
					get_keystatus[index]=1;					
				}
			if((figer_old[index]==1)&&(figer_new[index]==0))
				{
					//表示抬起
					get_keystatus[index]=0;
				}	
			if((figer_old[index]==0)&&(figer_new[index]==0))
				{
					//表示未按
					get_keystatus[index]=0;
					
				}	
			if((figer_old[index]==1)&&(figer_new[index]==1))
				{
					//表示一直按着未松开
					get_keystatus[index]=1;
				}	
			figer_old[index] = figer_new[index];
		}
	
	//printk("status_key[4] = %d,status_key[3] = %d, status_key[2] = %d,status_key[1]=%d,status_key[0]=%d\n", status_key[4], status_key[3], status_key[2],status_key[1],status_key[0]);
	//printk(">>>>touch_num=%d>>>>>\n", touch_num);
/*
 if(touch_num>0)
	{
...
*/
	//printk(">>>>touch_num=%d,key_num=%d>>>>>\n", touch_num,key_num);
	
		point_num = touch_num;  					//     1              2                3                2                      1                     0
		if(touch_num < key_num)	//				//   1 <-1       2 < 1       	    3 < 2          2 < 3                 1 < 2                 0<2
		{
			 point_num = key_num;// 			//   ;                ;                ;              point_num =3      point_num=2;    point_num =2
 			 touch_num = key_num;//  			//  ;                ;                ;               touch_num =3     touch_num=2;     touch_num =2
		}
		key_num = touch_num; 					//  1			2	           3             Key_num = 2        key_num =2;     key_num =2;



	for(index=0;index < touch_num;index++)
		{
			//printk(">>>>get_keystatus=%d>>>>>\n", get_keystatus[index]);

			if(get_keystatus[index]==1)
				{
					
					position = 4 - READ_COOR_ADDR + 5*index;
					input_buf[index][0] = ts->abs_x_max - ((unsigned int)(point_data[position]<<8) + (unsigned int)( point_data[position+1]));
					input_buf[index][1] = ts->abs_y_max - ((unsigned int)(point_data[position+2]<<8) + (unsigned int)(point_data[position+3]));
					input_buf[index][2] =(unsigned int) (point_data[position+4]);		
					//printk("input_x[%d] = %d,input_y = %d, input_w = %d\n", index,input_buf[index][0], input_buf[index][1], input_buf[index][2]);
					//printk(">>>>lqing anxia index=%d>>>>>\n",index);
					
					input_mt_slot(ts->input_dev, index);
					input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, index);
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X, input_buf[index][0]);
					input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, input_buf[index][1]);
					input_report_abs(ts->input_dev, ABS_MT_PRESSURE, input_buf[index][2]);
					
					
				}
			else				
				{
					//printk(">>>>>>>lqing taiqi index=%d\n", index);
					input_mt_slot(ts->input_dev, index);
...
static ssize_t touch_mode_show(struct class *cls,struct class_attribute *attr, char *_buf)
{
	...
	else
	{
	   calibration_flag = 0;
		...
		//if(calibration_flag == 1)
	//{
		//printk("=====touch_mode_show:calibration_flag ===== 1\n");
		//return sprintf(_buf,"successful");
	//}
	//else
	//{
		///printk("=====touch_mode_show:calibration_flag ===== 0\n");
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = class_create_file(tp_class,&class_attr_touchcalibration);
