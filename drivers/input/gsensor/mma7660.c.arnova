...
*/
	buffer[0] = MMA7660_REG_SR;
	buffer[1] = 0xf8 | (0x07 & MMA7660_RATE_120);
...
static void mma7660_report_value(struct i2c_client *client, struct mma7660_axis *axis)
{
    ...
    input_report_abs(mma7660->input_dev, ABS_X, axis->y);
    input_report_abs(mma7660->input_dev, ABS_Y, axis->z);
    input_report_abs(mma7660->input_dev, ABS_Z, -axis->x);
    input_sync(mma7660->input_dev);
    //printk("Gsensor x==%d  y==%d z==%d\n",axis->y,axis->z,-axis->x);
}

#define RawDataLength 3
int RawDataNum;
...
#endif
static  int  mma7660_shutdown(struct i2c_client *client)
{
	int ret = 0;
	printk("%s......gsensor......7660\n",__FUNCTION__);
	ret = mma7660_suspend(client,PMSG_SUSPEND);
