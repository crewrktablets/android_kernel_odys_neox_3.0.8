...
static int rk29_bl_update_status(struct backlight_device *bl)
{
	...
	if (ref) {
		...
		// divh = div_total*(BL_STEP-bl->props.brightness)/BL_STEP; Yatto Commented
                divh = div_total*(BL_STEP-bl->props.brightness)/BL_STEP+BACKLIGHT_REDUCE_VALUE;
                divh = divh*MIN_BACKLIGHT_SCALE/20; //Yatto Added
	}
