...
#if defined(CONFIG_GOODIX_CAPACITIVE_TOUCHSCREEN_TR810)
...
#define TOUCH_SHUTDOWN_PIN RK29_PIN6_PC3
#define TOUCH_INT_PIN   RK29_PIN0_PA2
int gt801_init_platform_hw(void)
{
	printk("%s,%d",__FUNCTION__,__LINE__);
    if(gpio_request(TOUCH_SHUTDOWN_PIN,NULL) != 0){
      gpio_free(TOUCH_SHUTDOWN_PIN);
      printk("gt801_init_platform_hw gpio_request error\n");
      return -EIO;
    }
	...
	gpio_direction_output(TOUCH_SHUTDOWN_PIN, GPIO_HIGH);
	msleep(100);
	gpio_set_value(TOUCH_SHUTDOWN_PIN, GPIO_LOW);
	msleep(100);
//	gpio_set_value(TOUCH_RESET_PIN,GPIO_HIGH);
//	mdelay(10);
//	gpio_set_value(TOUCH_RESET_PIN,GPIO_LOW);
...
void gt801_exit_platform_hw(void)
{
	printk("gt801_exit_platform_hw\n");
	gpio_free(TOUCH_SHUTDOWN_PIN);
	gpio_free(TOUCH_INT_PIN);
}

int gt801_platform_sleep(void)
{
	printk("gt801_platform_sleep\n");
	gpio_set_value(TOUCH_SHUTDOWN_PIN,GPIO_HIGH);
	return 0;
}

int gt801_platform_wakeup(void)
{
	printk("gt801_platform_wakeup\n");
	gpio_set_value(TOUCH_SHUTDOWN_PIN,GPIO_LOW);
	//msleep(5);
	//gpio_set_value(TOUCH_INT_PIN, GPIO_LOW); 
	//msleep(20);
...
struct goodix_platform_data gt801_info = {
  .model = 801,
  .irq_pin = TOUCH_INT_PIN,
  .init_platform_hw= gt801_init_platform_hw,
  .exit_platform_hw= gt801_exit_platform_hw,
  .platform_sleep  = gt801_platform_sleep,
  .platform_wakeup = gt801_platform_wakeup,
...
/* GT801 capts */
#ifdef CONFIG_GOODIX_CAPACITIVE_TOUCHSCREEN_TR810

#define TOUCH_ENABLE_PIN	RK29_PIN4_PD5
//#define TOUCH_ENABLE_PIN	RK29_PIN6_PC3
#define TOUCH_INT_PIN		RK29_PIN0_PA2
...
//Org	0x30,0x19,0x05,0x05,0x28,0x02,0x14,0x20,0x10,0x2D,0xB0,0x14,0x00,0x1E,0x00,0x01,
	0x30,0x19,0x05,0x05,0x28,0x02,0x14,0x20,0x10,0x2D,0xB0,
	TOUCH_MAX_WIDTH>>8,TOUCH_MAX_WIDTH&0xFF,TOUCH_MAX_HEIGHT>>8,TOUCH_MAX_HEIGHT&0xFF,0x01,
	0x23,0x45,0x67,0x89,0xAB,0xCD,0xE1,0x00,0x00,0x27,0x2C,
	0x4D,0xC4,0x20,0x05,0x03,0x83,0x50,0x3C,0x1E,0xB4,0x00,
	0x26,0x2B,0x01,0xA9,0x00,0x3C,0x32,0x71,0x00,0x00,0x00,
...
static struct goodix_i2c_rmi_platform_data ts_pdata = {
    ...
    .irq_edge = 0, /* 0:rising edge, 1:falling edge */
